...
import { seeded, type Random } from './rng';

const shadow = (r: number, dx = 0.7, dy = 1.1, opacity = 0.22) => (
  <circle cx={dx} cy={dy} r={r} fill={`rgba(50,18,6,${opacity})`} />
);

function dots(rand: Random, n: number, maxR: number, size: number, fill: string, opacity = 1): ReactElement[] {
...
const PEPPER_COLORS: string[] = ['#3d8b2e', '#3d8b2e', '#d8402e', '#eeb127'];

export interface ToppingArt {
  /** taxminiy radius (joylashtirishda to'qnashuvni hisoblash uchun) */
  r: number;
  /** butun pitsadagi soni */
  count: number;
  /** qatlam: 0 pishloq bilan birga, 1 go'sht, 2 sabzavot, 3 eng ustida */
  layer: 0 | 1 | 2 | 3;
  draw(rand: Random): ReactElement;
}

export const TOPPING_ART: Record<string, ToppingArt> = {
  pepperoni: {
    r: 9,
    count: 13,
    layer: 1,
  ...
  pepper: {
    ...
    layer: 2,
    draw: (rand: Random) => {
      const color = PEPPER_COLORS[Math.floor(rand() * PEPPER_COLORS.length)] ?? '#3d8b2e';
      return (
        <>
          <path
            d="M-6 2.2Q0 -4 6 2.2"
            fill="none"
            stroke="rgba(40,20,5,.22)"
            strokeWidth="3"
            strokeLinecap="round"
    ...
    draw: (rand: Random) => {
      ...
      return (
        <g transform={`scale(${s.toFixed(2)})`}>
          <path
            d="M0 -7.2C5.2 -4.4 5.2 3.4 0 7.2C-5.2 3.4 -5.2 -4.4 0 -7.2Z"
            fill="rgba(10,40,10,.22)"
            transform="translate(.6 .9)"
          />
  ...
  chili: {
    ...
    layer: 3,
    draw: (rand: Random) => (
      <path d={`M-1 -.6L${(0.6 + rand()).toFixed(1)} -.9L.9 .8Z`} fill={rand() > 0.4 ? '#c8261b' : '#8e1a10'} />
    ),
  },
};

// Belgi uchun ko'rinish maydoni (kattaroq son — kichikroq ko'rinadi)
const ICON_VIEW: Record<string, number> = {
  pepperoni: 10,
  qazi: 8.8,
  chicken: 8,
  beef: 5.8,
...
export function ToppingIcon({ id, size = 36 }: { id: string; size?: number }) {
