...
import { PIZZA_BY_ID, SAUCE_BY_ID } from '@shared/menu';
import { partToppings } from '@shared/pricing';
import type { CrustId, PizzaConfig, SauceId } from '@shared/types';
import { TOPPING_ART } from './toppings';
import { blobPath, scatter, seeded } from './rng';

const CRUST_INNER: Record<CrustId, number> = { classic: 82, thin: 88, cheese: 79 };

const SAUCE_STOPS: Record<SauceId, [string, string]> = {
...
function buildScene(config: PizzaConfig, seed: string) {
  const halves = config.parts.length === 2;
  const inner = CRUST_INNER[config.crust];
...
interface PizzaProps {
  config: PizzaConfig;
  size?: number;
  /** Bir xil seed — masalliqlar har doim bir xil joyda */
  seed?: string;
  /** masalliqlar tepadan "tushib" joylashadi (konstruktor) */
  animate?: boolean;
  /** yarim-yarim rejimida tanlangan yarmi, ikkinchisi xiraroq */
...
function PizzaImpl({
  ...
  const sauces = config.parts.map((part) => part.sauce ?? PIZZA_BY_ID[part.pizzaId]?.sauce ?? 'tomato');
  ...
  return (
    ...
    >
      ...
      {scene.peeks.map((p, i) => (
        <path key={i} d={p.d} fill={SAUCE_BY_ID[sauces[p.part] ?? 'tomato'].color} opacity=".6" />
      ))}
      {scene.bakes.map((p, i) => (
        <circle
          key={i}
      ...
      {scene.toppings.map((t) => (
        <g
          key={t.key}
          transform={`translate(${t.x.toFixed(1)} ${t.y.toFixed(1)}) rotate(${t.rot.toFixed(0)}) scale(${t.s.toFixed(2)})`}
        >
          ...
          <g
            ...
            style={animate ? { animationDelay: `${Math.min(t.order * 38 + t.layer * 45, 900)}ms` } : undefined}
          >
            {t.node}
          </g>
        </g>
