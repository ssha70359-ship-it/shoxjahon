...
import { defaultPizzaConfig, membersFromItems, splitBill } from '@shared/pricing';
import type { Lang, OrderDto, OrderStatus, PizzaConfig } from '@shared/types';
import { SHOP } from '@shared/shop';
import { STATUS_LABELS, customerCanCancel, trackerSteps } from '@shared/status';
import { LineView } from '../components/Cards';
import Icon from '../components/Icon';
import { OrderScene } from '../components/Oven';
import { Avatar, Spinner } from '../components/ui';
import { api } from '../lib/api';
import { formatDate, formatMoney, formatTime } from '../lib/format';
import { useNav, type ScreenParams } from '../lib/nav';
import { useStore } from '../lib/store';
import { callPhone, confirmDialog, hapticNotify, openInvoice } from '../lib/telegram';
...
const formatReached = (at: number | undefined) => (at == null ? '' : formatTime(at));

function Steps({ order, lang }: { order: OrderDto; lang: Lang }) {
  const steps = trackerSteps(order.mode);
  const reached = new Map<OrderStatus, number>(order.history.map((entry) => [entry.status, entry.at]));
        ...
        return (
          <li key={step} className={`${done ? 'is-done' : ''} ${current ? 'is-current' : ''}`}>
            <span className="steps__dot">
              {done && !current ? <Icon name="check" size={12} strokeWidth={3} /> : null}
            </span>
            <span className="steps__label">{tr(STATUS_LABELS[step], lang)}</span>
            <span className="steps__time">{formatReached(reached.get(step))}</span>
          </li>
        );
      })}
    </ol>
...
export default function Tracker({ params }: { params: ScreenParams['tracker'] }) {
  ...
  const firstPizza =
    order.items.map((item) => item.config).find((config): config is PizzaConfig => config.kind === 'pizza') ??
    FALLBACK_PIZZA;
  ...
  const shares = order.groupCode
    ? splitBill(membersFromItems(order.items), {
        ...
        {active && order.status !== 'pending_payment' && order.etaAt && (
          <div className="tracker__eta">
            <span className="tracker__eta-big">
              {minutesLeft != null && minutesLeft > 1 ? t('track.left', { n: minutesLeft }) : t('track.soon')}
            </span>
