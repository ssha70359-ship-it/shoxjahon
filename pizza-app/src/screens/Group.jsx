...
import { splitBill, summarize } from '../../shared/pricing.js';
import { LineView } from '../components/Cards.jsx';
import Icon from '../components/Icon.jsx';
import { Avatar, Money, Spinner, Stepper } from '../components/ui.jsx';
...
function Invite({ code }) {
  ...
  if (error) return <Intro note={t(`err.${error}`)} />;
  if (!group) {
    return (
...
export default function Group() {
  ...
  const countdown = useCountdown(group?.expiresAt ?? 0);
  const [busy, setBusy] = useState(false);
  ...
  return (
    <div className="screen group">
      ...
      {group.status === 'ordered' && (
        <button type="button" className="banner banner--order" onClick={() => nav.push('tracker', { id: group.orderId })}>
            ...
            {member.items.length === 0 ? (
              ...
              member.items.map((item) => (
                <LineView key={item.id} config={item.config} qty={item.qty} unit={item.unit} lang={lang}>
                  {editable ? <Stepper size="sm" value={item.qty} onChange={(qty) => changeQty(item.id, qty)} /> : <span className="muted">×{item.qty}</span>}
                </LineView>
              ))
            )}
          </section>
