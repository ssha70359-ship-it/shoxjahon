import { ITEMS, tr } from '../../shared/menu.js';
...
import { LoyaltyPizza } from '../components/PizzaStage.jsx';
...
export function EmptyBox({ title, text, action }) {
  return (
    <div className="empty">
...
export function SummaryRows({ summary, mode, lang, t }) {
  return (
...
export function FreeDelivery({ summary, lang, t }) {
...
export default function Cart() {
  ...
  const inCart = new Set(cart.filter((line) => line.config.kind === 'item').map((line) => line.config.itemId));
  const upsell = UPSELL.map((id) => ITEMS.find((item) => item.id === id)).filter(
    (item) => item && !inCart.has(item.id) && !state.stoplist.has(item.id),
  );
  ...
  return (
    <div className="screen cart">
      ...
      <div className="card lines">
        {cart.map((line) => (
          <LineView
            ...
            lang={lang}
            onOpen={
              line.config.kind === 'pizza'
                ? () => nav.push('builder', { config: line.config, lineKey: line.key })
                : undefined
            }
          >
      ...
      {upsell.length > 0 && (
        <>
          ...
          <div className="upsell">
            {upsell.map((item) => (
              <button key={item.id} type="button" className="upsell__item" onClick={() => actions.addLine({ kind: 'item', itemId: item.id })}>
      ...
      {blocked && <p className="note note--warn">{t('cart.minOrder', { sum: formatMoney(SHOP.delivery.minOrder, lang) })}</p>}

      <div className="action-bar action-bar--tab">
        <button type="button" className="btn btn--flame btn--grow btn--split" disabled={blocked} onClick={() => nav.push('checkout')}>
