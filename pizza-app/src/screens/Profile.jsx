...
import { assertAvailable } from '../../shared/pricing.js';
...
import { Avatar, Segmented, Spinner } from '../components/ui.jsx';
import { formatDate, formatMoney, formatPhone } from '../lib/format.js';
import { useNav } from '../lib/nav.jsx';
import { useStore } from '../lib/store.jsx';
import { addToHomeScreen, callPhone, canAddToHomeScreen } from '../lib/telegram.js';
import Pizza from '../pizza/Pizza.jsx';
import ItemArt from '../components/ItemArt.jsx';
import { ITEM_BY_ID } from '../../shared/menu.js';
...
export default function Profile() {
  ...
  const history = (state.historyIds ?? []).map((id) => state.orders[id]).filter(Boolean);

  function repeat(order) {
    const own = order.items.filter((item) => !item.by || item.by.id === user.id);
    const available = own.filter((item) => {
      try {
        assertAvailable(item.config, state.stoplist);
        return true;
      } catch {
        return false;
      }
