...
import { LIMITS, defaultPizzaConfig, normalizeLine, toppingPrice, unitPrice } from '../../shared/pricing.js';
import Icon from '../components/Icon.jsx';
import { PizzaStage } from '../components/PizzaStage.jsx';
import { Tags } from '../components/Cards.jsx';
import { Money, SectionTitle, Segmented, Stepper } from '../components/ui.jsx';
import { formatMoney } from '../lib/format.js';
import { useNav } from '../lib/nav.jsx';
import { useStore } from '../lib/store.jsx';
import { haptic, hapticNotify, hapticSelect } from '../lib/telegram.js';
import Pizza from '../pizza/Pizza.jsx';
import { ToppingIcon } from '../pizza/toppings.jsx';

const SIZE_SCALE = { S: 0.8, M: 0.9, L: 1 };
const HALF_CHOICES = [...PIZZAS.filter((pizza) => !pizza.hidden), PIZZA_BY_ID[CUSTOM_PIZZA_ID]];

function partFor(pizzaId) {
  return { pizzaId, sauce: PIZZA_BY_ID[pizzaId].sauce, removed: [], extras: [] };
}
...
function flyToCart(stageEl, tab) {
  ...
  const animation = clone.animate(
    [
      ...
      { transform: `translate(${dx * 0.35}px, ${dy * 0.2 - 80}px) scale(.45) rotate(120deg)`, opacity: 1, offset: 0.45 },
...
export default function Builder({ params }) {
  ...
  const part = config.parts[Math.min(activePart, config.parts.length - 1)];
  ...
  const title = halves
    ? config.parts.map((p) => tr(PIZZA_BY_ID[p.pizzaId].name, lang)).join(' / ')
    : tr(pizza.name, lang);
  ...
  function updatePart(patch) {
    setConfig((current) => ({
      ...current,
      parts: current.parts.map((p, i) => (i === Math.min(activePart, current.parts.length - 1) ? { ...p, ...patch } : p)),
    }));
  }
  ...
  function choosePizzaForPart(pizzaId) {
    hapticSelect();
    ...
    <div className="screen builder">
      ...
      <div className="builder__sheet">
        ...
        <Segmented
          ...
          value={config.crust}
          onChange={(value) => update({ crust: value })}
          options={CRUSTS.map((crust) => ({
            value: crust.id,
            label: tr(crust.name, lang),
            hint: crust.price[config.size] ? `+${formatMoney(crust.price[config.size], lang, { withCurrency: false })}` : null,
          }))}
        />

        <button type="button" className={`half-toggle ${halves ? 'is-on' : ''}`} onClick={toggleHalf} aria-pressed={halves}>
            ...
            <div className="half-panel__tabs">
              {config.parts.map((p, i) => (
                ...
                >
                  ...
                  <b>{tr(PIZZA_BY_ID[p.pizzaId].name, lang)}</b>
                </button>
              ))}
            </div>
              ...
              {pizza.toppings.map((id) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span>{tr(TOPPING_BY_ID[id].name, lang)}</span>
          ...
          <div key={group.id} className="extras-group">
            ...
            <div className="extras">
              {TOPPINGS.filter((topping) => topping.group === group.id).map((topping) => {
                ...
                const shown = halves ? Math.round(price / 2000) * 1000 : price;
                return (
                  <button
                    key={topping.id}
                    type="button"
      ...
      <div className="action-bar">
        ...
        <button type="button" className="btn btn--flame btn--grow btn--split" onClick={submit} disabled={busy || soldOut}>
          <span>
            {soldOut
              ? t('common.soldOut')
              : editing
