...
import { CATEGORIES, ITEMS, PIZZAS, TOPPING_BY_ID, tr } from '../../shared/menu.js';
...
import { ItemRow, PizzaCard } from '../components/Cards.jsx';
import Icon from '../components/Icon.jsx';
import { LoyaltyPizza, PizzaStage } from '../components/PizzaStage.jsx';
import ShakeOverlay from '../components/ShakeOverlay.jsx';
import { Segmented } from '../components/ui.jsx';
import { formatMoney } from '../lib/format.js';
import { useNav } from '../lib/nav.jsx';
import { canShake, useShake } from '../lib/sensors.js';
import { useStore } from '../lib/store.jsx';
import { haptic } from '../lib/telegram.js';

const VISIBLE_PIZZAS = PIZZAS.filter((pizza) => !pizza.hidden);
const HERO = VISIBLE_PIZZAS.find((pizza) => pizza.tags.includes('signature')) ?? VISIBLE_PIZZAS[0];
  ...
  return (
    <>
      {activeOrders.slice(0, 2).map((order) => (
        <button key={order.id} type="button" className="banner banner--order" onClick={() => nav.push('tracker', { id: order.id })}>
...
function QuickActions({ onShake }) {
  ...
  return (
    <section className="quick">
      {actions.map((action) => (
        <button key={action.id} type="button" className={`quick__item quick__item--${action.id}`} onClick={action.onClick}>
...
function LoyaltyStrip() {
  ...
  return (
    <button type="button" className={`loyalty-strip ${ready ? 'is-ready' : ''}`} onClick={() => nav.setTab(ready ? 'cart' : 'profile')}>
...
export default function Home() {
  ...
  const openPizza = useCallback((pizzaId) => nav.push('builder', { pizzaId }), [nav]);
  ...
  const pizzas = useMemo(() => {
    ...
    return VISIBLE_PIZZAS.filter(
      (pizza) =>
        matches(tr(pizza.name, lang), q) ||
        pizza.toppings.some((id) => matches(tr(TOPPING_BY_ID[id].name, lang), q)),
    );
  }, [q, lang]);
  ...
  const pizzaGrid = (
    <div className="pizza-grid">
      {pizzas.map((pizza) => (
        <PizzaCard key={pizza.id} pizza={pizza} lang={lang} t={t} soldOut={state.stoplist.has(pizza.id)} onOpen={openPizza} />
      ))}
    </div>
  );
