...
function Bottle({ color, label }) {
  return (
    <>
      <Shadow rx={20} />
      <path
        d="M53 20h14v8c0 5 11 10 11 24v48c0 4-3 7-7 7H49c-4 0-7-3-7-7V52c0-14 11-19 11-24z"
        fill={color}
      />
      ...
      <path d="M48 36c-2 4-4 7-4 14v44" stroke="#fff" strokeOpacity=".4" strokeWidth="3" strokeLinecap="round" fill="none" />
    </>
  );
}
...
function Fries() {
  ...
  return (
    <>
      <Shadow rx={28} />
      {sticks.map(([x, y, r], i) => (
        <rect key={i} x={x - 3.5} y={y} width="7.5" height="52" rx="2" fill={i % 2 ? '#f6c649' : '#f2b83a'} stroke="#d99a25" strokeWidth=".8" transform={`rotate(${r} ${x} 70)`} />
      ))}
...
function Wings() {
  const wing = (x, y, r) => (
    <g transform={`translate(${x} ${y}) rotate(${r})`}>
      ...
      <path d="M-14-4c6-7 20-8 25-2" stroke="#e08a44" strokeWidth="2.4" strokeLinecap="round" fill="none" opacity=".8" />
...
function Dip({ color }) {
  return (
    <>
      ...
      <path d="M48 54c6-3 14-3 22 0" stroke="#fff" strokeOpacity=".45" strokeWidth="2" strokeLinecap="round" fill="none" />
...
function ItemArtImpl({ art, size = 72, label }) {
  const Component = ART[art?.type] ?? Dip;
  return (
