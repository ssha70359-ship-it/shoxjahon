    ...
    <svg viewBox="0 0 360 220" className="scene__svg" aria-hidden="true">
      ...
      <g className="scene__flour">
        {[
          ...
          <circle key={i} cx={x} cy={y} r={2 + (i % 3)} fill="#fff" opacity=".85" style={{ animationDelay: `${i * 0.25}s` }} />
        ))}
      </g>
    </svg>
  );
...
function Road() {
  const skyline = (y, color, className) => (
...
export function OrderScene({ order, pizza }) {
